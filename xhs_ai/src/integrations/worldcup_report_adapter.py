...
DISCLAIMER = "以上内容为模型概率预测，不代表确定赛果，不构成任何投注建议。"
REQUIRED_TAGS = ["#世界杯", "#世界杯预测", "#足球", "#AI预测", "#数据分析"]
...
class WorldCupReportAdapter:
    ...
    def build_post_draft(self, report: Dict[str, Any]) -> WorldCupPostDraft:
        ...
        findings = self._collect_key_findings(report.get("agents") or [])
        warnings = self._string_list(consensus.get("warnings"))
        completeness, data_notes = self._data_completeness(report.get("intelligence"))
        kickoff = str(fixture["kickoff"]).strip()
        stage = str(fixture["stage"]).strip()
        ...
        basis_lines = findings or ["报告未提供可用的 Agent 关键判断依据。"]
        warning_lines = warnings or data_notes or ["未发现额外警告。"]
        content_parts = [
            f"{home} vs {away}，这场世界杯比赛的多 Agent 预测已经汇总。",
            fact_block,
            "【关键判断】\n"
            + "\n".join(f"{index}. {item}" for index, item in enumerate(basis_lines, 1)),
            f"【数据状态】\n数据完整度：{self._format_percent(completeness)}\n"
        ...
        return WorldCupPostDraft(
            title=f"{home}vs{away} 世界杯AI预测"[:20],
            content="\n\n".join(content_parts),
            topic=f"{home} vs {away} 世界杯预测",
            metadata=metadata,
            ...
            "\n".join(
                [
                    ...
                    *[f"{index}. {item}" for index, item in enumerate(findings, 1)],
                    f"Agent一致率：{self._format_optional_percent(facts['agreement_score'])}",
                    f"风险提示：{risk}",
                ]
            ),
        ]
    ...
    def _build_fact_block(self, facts: Dict[str, Any]) -> str:
        ...
        return "\n".join(
            [
                FACT_BLOCK_START,
                f"比赛：{facts['home_team']} vs {facts['away_team']}",
                f"阶段：{facts['stage']}",
                ...
                f"客胜概率：{self._format_percent(facts['away_win_probability'])}",
                f"Agent一致率：{self._format_optional_percent(facts['agreement_score'])}",
                f"Agent数量：{facts['agent_count'] if facts['agent_count'] is not None else '未知'}",
                FACT_BLOCK_END,
            ]
        )
    ...
    def _parse_fact_block(self, content: str) -> Dict[str, Any] | None:
        ...
        if not required.issubset(values):
            return None
        teams = re.fullmatch(r"(.+?)\s+vs\s+(.+)", values["比赛"])
        ...
        try:
            return {
                ...
                "agreement_score": (
                    None
                    if values["Agent一致率"] == "未知"
                    else self._percent_to_float(values["Agent一致率"])
                ),
                "agent_count": (
                    None if values["Agent数量"] == "未知" else int(values["Agent数量"])
                ),
            }
